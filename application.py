...
def login():
    ...
    if request.method == "POST":
        ...
        users_info = db.execute("SELECT * FROM users WHERE username = :username", username=username)
...
def register():
    ...
    if request.method == "POST":
        ...
        user = db.execute("SELECT username FROM users WHERE username = :username", username=username_input)
        if len(user) != 0:
...
def get_transaction_param():
    ...
    cash = get_query_with_key(db.execute("SELECT cash FROM users WHERE id = {id}", id=session['user_id']), "cash")
